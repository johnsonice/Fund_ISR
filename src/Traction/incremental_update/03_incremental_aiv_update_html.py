...
from data_preprocess import doc_to_paragraphs  # noqa: E402
from data_preprocess_html import extract_StaffAppraisal_and_Buff_html  # noqa: E402

DEFAULT_RAW_HTML_ROOT = Path(
    "/data/home/xiong/data/Fund/CSR/Tractions/ArticleIV_xml_updated/06_07_2026_update/imf_staff_reports_html_by_year_isbn"
)
DEFAULT_METADATA_PATH = Path(
    "/data/home/xiong/data/Fund/CSR/Tractions/output/incremental_update/06_07_2026_update/imf_staff_reports_html_all_metadata_postprocessed_2015plus.xlsx"
)
DEFAULT_OUTPUT_DIR = config.output_dir / "incremental_update" / "06_07_2026_update_html"


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run an incremental AIV extraction on nested HTML update folders using a filtered metadata workbook."
    )
    parser.add_argument(
        "--raw-html-root",
        type=Path,
        default=DEFAULT_RAW_HTML_ROOT,
        help=f"Nested HTML update root. Default: {DEFAULT_RAW_HTML_ROOT}",
    )
    parser.add_argument(
        "--metadata-path",
        type=Path,
...
def discover_package_folders(raw_html_root: Path) -> tuple[dict[str, str], dict[str, list[str]]]:
    folder_dict: dict[str, str] = {}
    duplicates: dict[str, list[str]] = {}

    for candidate in sorted((path for path in raw_html_root.rglob("*") if path.is_dir()), key=lambda p: str(p)):
        try:
            child_htmls = [
                child
...
def _preserve_or_derive(df: pd.DataFrame, target_col: str, derived: pd.Series) -> pd.Series:
    """Keep curated values when present and non-empty; fall back to `derived` otherwise."""
    if target_col not in df.columns:
        return derived
    existing = df[target_col].astype(str).where(df[target_col].notna(), "").str.strip()
    return existing.mask(existing == "", derived)
...
def load_metadata(metadata_path: Path) -> pd.DataFrame:
    ...
    derived_code = df_meta.get("country_codes", pd.Series([""] * len(df_meta))).apply(_split_pipe_first)
    derived_desc = df_meta.get("countries", pd.Series([""] * len(df_meta))).apply(_split_pipe_first)
    df_meta["Primary Country Code"] = _preserve_or_derive(df_meta, "Primary Country Code", derived_code)
    df_meta["Primary Country Description"] = _preserve_or_derive(df_meta, "Primary Country Description", derived_desc)
...
def build_documents_df(df_doc: pd.DataFrame) -> pd.DataFrame:
    """Per-document text rollup matching the legacy df_documents.csv schema.

    Drops rows where either the staff or buff text is empty after joining.
    """
    rows = []
    for _, r in df_doc.iterrows():
        ...
        if not staff.strip() or not buff.strip():
            continue
...
def write_summary(
    ...
    summary = {
        "raw_html_root": str(raw_html_root),
...
def main(argv: list[str] | None = None) -> None:
    ...
    raw_html_root = args.raw_html_root.resolve()
    metadata_path = args.metadata_path.resolve()
    output_dir = args.output_dir.resolve()
    output_dir.mkdir(parents=True, exist_ok=True)

    folder_dict_all, duplicates = discover_package_folders(raw_html_root)
    if not folder_dict_all:
        raise FileNotFoundError(f"No package folders with HTML files found under {raw_html_root}")
    ...
    (
        ...
        para_av_dict,
        av_uncertain_dict,
    ) = extract_StaffAppraisal_and_Buff_html(selected_folder_dict)
