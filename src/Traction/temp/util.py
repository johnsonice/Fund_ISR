...
# Ensure imports and `.env` loading work regardless of current working directory.
# File is: <repo_root>/src/Traction/temp/util.py  -> repo_root is 3 parents up.
_REPO_ROOT = Path(__file__).resolve().parents[3]
sys.path.insert(0, str(_REPO_ROOT))

from dotenv import load_dotenv, find_dotenv

_dotenv_path = find_dotenv(".env", raise_error_if_not_found=False)
if _dotenv_path:
    load_dotenv(_dotenv_path, override=False)
else:
    # Fallback: assume `.env` is at repo root
    load_dotenv(_REPO_ROOT / ".env", override=False)

api_key = os.getenv("OPENAI_API_KEY")
if not api_key:
    raise RuntimeError(
        "OPENAI_API_KEY is not set. Provide it via environment or add it to "
        f"{_REPO_ROOT / '.env'}"
    )
#%%
