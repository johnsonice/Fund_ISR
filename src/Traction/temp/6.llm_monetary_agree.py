...
#%%
# save
output_dir = Path(config.data_dir / 'Finetuning_data/llm_evaluation/monetary/df_agree_gpt_v1.xlsx')
...
# df_train_agree[['messages']].to_json(directory / 'Finetuning_data/monetary/cv/training_mon_agree_%d.jsonl'%idx, orient='records', lines=True)
# # df_train_agree[['messages']].to_json(directory+'training_mon_agree.jsonl', orient='records', lines=True)

# # check fine-tuning dataset
# check_finetuning_dataset(directory / 'Finetuning_data/monetary/cv/training_mon_agree_%d.jsonl'%idx)


# # 5.3. create fine-tuning job
# file = client.files.create(
#     file=open(directory / 'Finetuning_data/monetary/cv/training_mon_agree_%d.jsonl'%idx, "rb"),
...
# # save results
# df_test_agree.drop('error_gpt_ft4', axis=1).to_excel(directory / 'Finetuning_data/llm_evaluation/monetary/df_test_agree_mon_%d.xlsx'%idx, index=False)
...
# df_agree[['messages']].to_json(directory / 'Finetuning_data/monetary/all_mon_agree.jsonl', orient='records', lines=True)

# check_finetuning_dataset(directory / 'Finetuning_data/monetary/all_mon_agree.jsonl')

# # create finetuning job
# file = client.files.create(
#     file=open(directory / 'Finetuning_data/monetary/all_mon_agree.jsonl', "rb"),
#     purpose="fine-tune"
# )
# print(file.id)
