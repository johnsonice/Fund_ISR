...
idx = 0
df_train = pd.read_excel(directory / f'Finetuning_data/Monetary/cv/training_{idx}.xlsx')
df_test = pd.read_excel(directory / f'Finetuning_data/Monetary/cv/testing_{idx}.xlsx')
...
df_train_stance[['messages']].to_json(directory / f'Finetuning_data/Monetary/cv/training_mon_{idx}.jsonl', orient='records', lines=True)

# check fine-tuning dataset
check_finetuning_dataset(directory / f'Finetuning_data/Monetary/cv/training_mon_{idx}.jsonl')

#%%

# 5.3. create fine-tuning job
file = client.files.create(
    file=open(directory / f'Finetuning_data/Monetary/cv/training_mon_{idx}.jsonl', "rb"),
...
#ftjob-BD4zfvfKLTYIM2wH03DWe8vQ
client.fine_tuning.jobs.list_events(fine_tuning_job_id="ftjob-BD4zfvfKLTYIM2wH03DWe8vQ", limit=3)
