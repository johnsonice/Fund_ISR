...
# query v1:
print('raw:', accuracy_score(df_stance['stance_current'], df_stance['stance_current_gpt1']), accuracy_score(df_stance['stance_future'], df_stance['stance_future_gpt1']), f1_score(df_stance['stance_current'], df_stance['stance_current_gpt1'], average='weighted'), f1_score(df_stance['stance_future'], df_stance['stance_future_gpt1'], average='weighted'), 'merging unclear/irrelevant:', accuracy_score(df_stance['stance_current'].apply(lambda x: 'unclear' if x=='irrelevant' else x), df_stance['stance_current_gpt1'].apply(lambda x: 'unclear' if x=='irrelevant' else x)), accuracy_score(df_stance['stance_future'].apply(lambda x: 'unclear' if x=='irrelevant' else x), df_stance['stance_future_gpt1'].apply(lambda x: 'unclear' if x=='irrelevant' else x)), f1_score(df_stance['stance_current'].apply(lambda x: 'unclear' if x=='irrelevant' else x), df_stance['stance_current_gpt1'].apply(lambda x: 'unclear' if x=='irrelevant' else x), average='weighted'), f1_score(df_stance['stance_future'].apply(lambda x: 'unclear' if x=='irrelevant' else x), df_stance['stance_future_gpt1'].apply(lambda x: 'unclear' if x=='irrelevant' else x), average='weighted'))
...
# save
df_stance.to_excel(directory+'llm_evaluation/monetary/df_train_stance_gpt.xlsx', index=False)
...
df_train_stance[['messages']].to_json(directory+'finetuning/monetary/cv/training_mon_%d.jsonl'%idx, orient='records', lines=True)

# check fine-tuning dataset
check_finetuning_dataset(directory+'finetuning/monetary/cv/training_mon_%d.jsonl'%idx)


# 5.3. create fine-tuning job
file = client.files.create(
    file=open(directory+'cv/training_mon_%d.jsonl'%idx, "rb"),
...
# save results
df_test_stance.to_excel(directory+'llm_evaluation/monetary/df_test_stance_mon_%d.xlsx'%idx, index=False)
...
df_stance[['messages']].to_json(directory+'finetuning/monetary/all_mon.jsonl', orient='records', lines=True)

check_finetuning_dataset(directory+'finetuning/monetary/all_mon.jsonl')

# create finetuning job
file = client.files.create(
    file=open(directory+'all_mon.jsonl', "rb"),
    purpose="fine-tune"
)
print(file.id)
